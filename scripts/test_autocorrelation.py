...
def save_uncorrelated_timeseries(T, cols_to_copy, L, L_equilibrated_min_value, Bex, folderName, mech, folder='../analysis'):
    for l in L:
...
def calc_max_tau(T, to_check, L, L_equilibrated_min_value, Bex, folderName, mech, folder='../analysis'):
...
def main():
    ...
    fname='../analysis/'+folderName+'/table_'+str(L)+'_'+str(L)+'_'+str(h_ex)+'_'+str(T)+'_'+mech+'.txt'
    y = pd.read_csv(fname, delim_whitespace=True, error_bad_lines=False, index_col='index', comment='#')
