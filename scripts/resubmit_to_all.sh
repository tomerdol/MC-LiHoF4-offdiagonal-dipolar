...
count=0
while [[ $num_of_queued_jobs -gt 0 && count -lt 10 ]]
do
    ...
    qsub -l mem_free=40G -V -S /bin/bash -cwd -N "$jobname" -o ./output/ -e ./output/ -q smoshe.q,lublin.q,intel_all.q scripts/met_with_t_single.sh "${args//,/ }"
    qdel "$jobid"
