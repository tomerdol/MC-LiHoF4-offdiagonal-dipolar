...
def main_plot(L, L_equilibrated_min_value, tau_dict, boot_num, h_ex, mech, folderName, xdata, folder='../analysis', start_in_middle=True):
    all_yboot=[]
    Nsigma=1.
    markers=['o','s','^','D','v']
    toplot=[]
    ...
    plt.tight_layout()
    axes = plt.gca()
    fig.savefig('./graphs/plot_%s_%s_%s.png'%(h_ex,mech,'_'.join(map(str,folderName))))
    plt.close()
