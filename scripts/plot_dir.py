...
def main_plot(L, L_equilibrated_min_value, tau_dict, boot_num, h_ex, mech, folderName, xdata, folder='../data/results', start_in_middle=True):
    all_yboot=[]
    Nsigma=1.
    markers=['o','s','^','D','v']
    toplot=[]
    ...
    print(xdata)
    all_y_curves=[]
    for i, fldrName in enumerate(folderName):
        y_to_plot=[]
        y_err_to_plot=[]
        data_label = 'int=%s' %fldrName
        
        for T in xdata[fldrName]:
            fname=folder+'/'+fldrName+'/table_'+str(L)+'_'+str(L)+'_'+str(h_ex)+'_'+str(T)+'_'+mech+'.txt'
            ...
            del mag
            del energy
        print('Curve to be plotted (L=%s, foldr=%s): '%(L,fldrName))
        print(y_to_plot)
        all_y_curves.append(y_to_plot)
        plt.errorbar(xdata[fldrName],y_to_plot,yerr=y_err_to_plot, fmt=markers[i % len(markers)]+'-', capsize=3, label=data_label)
    ax = plt.gca()
    #ax.set_yscale("log")
    ...
    plt.tight_layout()
    axes = plt.gca()
    fig.savefig('../figures/plot_%s_%s_%s.png'%(h_ex,mech,'_'.join(map(str,folderName))))
    plt.close()
...
def main():
    ...
    for folderName in folderName_list:
        with open('../temp_schedule_' + folderName + '_' + str(h_ex) + '_' + mech + '.txt','r') as temperature_schedule_file:
            reader = csv.reader(temperature_schedule_file)
            temp_list=list(reader) 
        xdata[folderName]=temp_list[0]
        xdata[folderName]=[float(i) for i in xdata[folderName]]
    
    main_plot(all_L[0], L_equilibrated_min_value, tau_dict, boot_num, h_ex, mech, folderName_list, xdata, folder=folder)
